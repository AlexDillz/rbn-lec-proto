...
### `index.html`
Главная точка входа проекта. Загружает список лекций из `lectures.json` и собирает карточки на странице.

### `lectures.json`
Конфиг со списком лекций, в котором для каждой записи хранятся:
- название
- краткое описание
- ссылка на страницу
- статус доступности
...
### `style.css`
Общий стиль сайта: главная страница, карточки, лекции, вспомогательные разделы и адаптивность.

### `theme.js`
Переключение светлой и тёмной темы с сохранением выбора пользователя.

### `protect-local.js`
Локальная защита и логика ограниченного просмотра отдельных страниц.
